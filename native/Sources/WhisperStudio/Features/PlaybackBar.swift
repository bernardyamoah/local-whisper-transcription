        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 16) {
                ...
                Button(controller.playing ? "Pause" : "Play", systemImage: controller.playing ? "pause.fill" : "play.fill", action: controller.toggle)
                    .labelStyle(.iconOnly).studioButton(prominent: true).buttonBorderShape(.circle).controlSize(.regular)
                ...
                }.labelsHidden().frame(width: 72).onChange(of: controller.speed) { controller.setSpeed() }
