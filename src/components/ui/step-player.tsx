...
const morphBetween = (from: IconState): Interpolator =>
  from === "play"
    ? separate(PLAY_PATH, PAUSE_SHAPES, MORPH_OPTIONS)
    : combine(PAUSE_SHAPES, PLAY_PATH, MORPH_OPTIONS);
    ...
    >
      {showControl && (
        <motion.button
          ...
          className="flex shrink-0 cursor-pointer touch-manipulation select-none items-center justify-center rounded-full bg-[#fffdf8] outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#476653]"
        >
          <TransportIcon
            state={iconState}
            size={metrics.icon}
      ...
      <div
        ...
        className="flex flex-1 items-center justify-center rounded-full bg-[#fffdf8]"
      >
        {items.map((step, i) => {
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                state === "past" ? "bg-[#476653]" : "bg-[#9bae9e]",
                seekable &&
            ...
            >
              {state === "active" && (
                <motion.span
                  ...
                  className="absolute inset-y-0 left-0 bg-[#476653]"
                />
              )}
              {seekable && (
                <button
