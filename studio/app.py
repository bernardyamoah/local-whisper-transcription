...
class Settings(BaseModel):
    ...
    hardware: Literal["auto", "cpu", "cuda"] = "auto"
    ...
    def language_supported(cls, value):
        ...
        if value != "auto" and value not in _LANGUAGE_CODES:
...
def create_app(root=None, worker=True, command=None):
    ...
    app.state.store, app.state.jobs, app.state.models = store, jobs, models
    ...
    def environment(request: Request):
        ...
        return {
            ...
            "origin": "healthy",
            "tunnel": "verified request" if request.state.access_verified else "not verified from localhost",
        }
    ...
    async def upload(request: Request):
        ...
        try:
            ...
            with store.connect() as db:
                db.execute(
                    "INSERT INTO media(id,name,size,duration,container,codec,checksum,created) VALUES(?,?,?,?,?,?,?,?)",
                    (
                        identifier,
                        name,
                        size,
    ...
    def create_job(payload: NewJob):
        ...
        return jobs.create(payload.media_id, payload.title.strip(), payload.language, payload.preset, model)
    ...
    def history(q: str = "", sort: str = "newest", offset: int = 0, limit: int = 50):
        ...
        query = " FROM jobs j JOIN media m ON j.media_id=m.id WHERE j.title LIKE ? OR m.name LIKE ?"
        with store.connect() as db:
            args = (f"%{q[:200]}%",) * 2
    ...
    def retry(identifier: str):
        job = store.job(identifier)
        if not models.status(job["model"])["installed"]:
    ...
    def audio(identifier: str):
        ...
        if not job["playback_available"] or job["state"] != "completed":
            raise HTTPException(
